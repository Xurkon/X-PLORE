...
function Tabs:HandleClick()
    if self.isActive then
        -- Already active: right-clicking could show menu; left-click does nothing extra
        return
    end
    self:ActivateGuide()
end
...
function Tabs:ShowContextMenu()
    -- For now, just remove the tab on right-click (simple implementation)
    -- Later: build a proper dropdown menu like Zygor (recent guides, suggested, etc.)
    self:RemoveTab()
end
...
function Tabs:ReanchorTabs()
    ...
    if count == 0 then
        -- No tabs: just position the add button
        if Tabs.AddButton then
            Tabs.AddButton:ClearAllPoints()
            Tabs.AddButton:SetPoint("LEFT", Tabs.container, "LEFT", 4, 0)
        end
        return
    end
    ...
    local availWidth = containerWidth - addBtnWidth - 8 -- margins
    ...
    for _, tab in ipairs(Pool) do
        if tab.guideID then
            ...
            if prev then
                ...
                tab.Button:SetPoint("LEFT", Tabs.container, "LEFT", 2, 0)
            end
