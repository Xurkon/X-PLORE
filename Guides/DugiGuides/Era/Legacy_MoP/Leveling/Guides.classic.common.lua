...
DGV.Guides = Guides
GUIUtils = GUIUtils or DGV.GUIUtils
...
--Calendar_LoadUI and C_Calendar not present.  Todo: probably remove
	...
	local tabs = {
		...
		[Category.Leveling] = {text = "Leveling", 	title = "Leveling Guides", 	guidetype = "L",	LeftFrame = DGVHomeFrame, 		RightFrame = DGVScrollFrame4,	icon="Interface\\Icons\\Achievement_Level_90", rightShouldScroll = false},
	...
	local function TabInfoActivate_(self, isInThread)
        ...
        if self.text == "Recent Guides" then
            if not whatsNewFrame then
                ...
                local title = guideategorieswrapper:CreateFontString(nil, "ARTWORK", whatsNewTitleFont)
...
--GetQuestObjectiveInfo not present.  Todo: check if needed/find replacement 
	...
	local function EvaluatePHA(pha)
		...
		local stage = (C_MapBar and C_MapBar.BarIsShown() and C_MapBar.GetPhaseIndex() + 1)  or  0
		if isCurrentMapIsleOfThunder then
			return tonumber(pha) <= stage 
		end 
	end
	...
	function DGV:ShouldSuggestNewGuide(playerLevel, checkIfWasAlreadySuggested)
		...
		if mapId == 202 or mapId == 180 or mapId == 179 or mapId == 180 or mapId == 181 or mapId == 182 or mapId == 605 or mapId == 174 or mapId == 175 or mapId == 176 or mapId == 177 or mapId == 178 or mapId == 124 then return end 
	...
	function DGV:GetSuggestedGuide(playerLevel)
		...
		if DGV.gtype and DGV.gtype[CurrentTitle] == "L" then suggestion = _CycleThroughGuides(self:GetRawTitle(CurrentTitle), playerLevel) end
		...
		local startguides = 
		{
				BloodElf = "94(1-12 Blood Elf)", 
				Orc = "1(1-5 Orc)", 
				Troll = "1(1-5 Troll)", 
				Goblin = "194(5-12 Goblin)",
				Tauren = "7(1-10 Tauren)", 
				Scourge = "18(1-10 Undead)", 
				Undead = "18(1-10 Undead)", 
				Dwarf = "27(1-5 Dwarf)", 
				Gnome = "27(1-5 Gnome)",
				Draenei = "97(1-12 Draenei)", 
				Human = "37(1-10 Human)", 
				NightElf = "57(1-10 Night Elf)",
				Worgen = "179(1-12 Worgen)", 
				DeathKnight = "124(55-58 Death Knight)",
...
-- 					if i == #XYVals then TomTom:SetCrazyArrow(DGV.DugisArrow.waypoints[1].tomtom, 5, desc) DGV.DugisArrow:Hide() end
	...
	local C_CurrencyInfo = C_CurrencyInfo or C_CurrencyInfo.GetCurrencyInfo
...
--[[	
	...
	function DGV:MoveToNextQuest(MoveToIndex, isInThread, recursion, fromOnModulesLoaded, fromSeverUpdate)
		...
		if DGU.CurrentQuestIndex <= lastUsedStepIndex then
			...
			if (not DGV:ReturnTag("AYG", DGU.CurrentQuestIndex) and checkMoved ~= DGU.CurrentQuestIndex) or 
				DGV:ReturnTag("AYG", DGU.CurrentQuestIndex) or 
				MoveToIndex then
	...
	function DGV:DisplayViewTab(title, skip, threading, autoScrollToCurrent, fromOnModulesLoaded)
		if not DugisGuideViewer:IsGoldMode() then return end
	...
	local function GetCreateTabRow(tabNum, rowNum, title, originalTabNum)
		...
		if not row then
			...
			row:SetNormalTexture("Interface\\LFGFRAME\\UI-LFG-BACKGROUND-GENERICDUNGEON");
    ...
    function DGV:UpdateRowVisualizations(i)
        ...
        if i == DGU.CurrentQuestIndex then
            ...
            frame:SetNormalTexture("Interface\\LFGFRAME\\UI-LFG-BACKGROUND-GENERICDUNGEON")   
        end
        ...
        if rowObj.currentTitle ~= CurrentTitle then
            ...
            DGV:SetQuestText(i)
            local texture = DGV:getIcon(DGV.actions[i], i) 
			if texture then 
		...
		for i = 4, #tabs do
			...
			if visualControlMap[i] then
				...
				if TabInfo.guidetype and not self.guidelist[TabInfo.guidetype] then
					...
					local dX = 0
					local yIndex
					if i == Category.Help or i == Category.Elites or i == Category.Mounts or i == Category.Pets or i == Category.Bosses then
