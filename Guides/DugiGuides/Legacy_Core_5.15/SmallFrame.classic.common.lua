...
local SmallFrame, UIFrameCache = DGV:RegisterModule("SmallFrame", "UIFrameCache")--, "Guides")
...
function SmallFrame:Initialize()
	...
	local function StatusFrame_SetCurrentWidth(frame, frameWidth, textWidth)
		...
		if textWidth then
			frame.Text:SetWidth(textWidth - 80)
	...
	local function StatusFrame_Layout(frame)
		...
		if frame.Desc.isHtml then
			frame.Desc:SetJustifyH("P", "LEFT")
			frame.DescEventHandler:SetJustifyH("P", "LEFT")
		else
	...
	local function SetPointAbsolute(region, point, relativeRegion, relativeRegionAbsoluteX, relativeRegionAbsoluteY, offsetX, offsetY, parent)
		...
		region:SetParent(parent or UIParent)
		if DGV:IsSmallFrameFloating() then
	...
	local function MoveItemButton(reaction, frame)
		...
		SetPointAbsolute(frame.ItemButton, "TOPRIGHT", frame.Objectives, frame.Objectives:GetRight(), frame.Objectives:GetTop(), 10, -2, frame)
	end
	...
	local function StatusFrame_Populate(frame, stepIndex)
		...
		if stepIndex and DGV.actions[stepIndex] then
			...
			local texture = DGV:getIcon(DGV.actions[stepIndex], stepIndex)
			if texture then 
            ...
            if frame.htmlDesc == nil then
                ...
                frame.htmlDesc:SetFontObject("P", frame.Desc:GetFontObject())
                ...
                frame.htmlDescEventHandler:SetFontObject("P", frame.Desc:GetFontObject())
	...
	function SmallFrame:OnDragStart()
		if DGV:IsSmallFrameAnchored() and not DGV:UserSetting(DGV_MOVEWATCHFRAME) then
			return
		end
		
	 	if DGV:IsSmallFrameFloating() and DGV:UserSetting(DGV_LOCKSMALLFRAME) then
			return
		end
...
--[[					
	...
	SmallFrame.Load = function()
		statusFrames = UIFrameCache:New("FRAME", "DugisSmallFrameStatus", DugisSmallFrameContainer, "DugisSmallFrameTemplate")
		...
		function DGV:HideSmallframe()
			if not DugisGuideViewer:IsGoldMode() then
				SmallFrameBkg:Hide()
				if SmallFrame.Frame then 
					SmallFrame.Frame:Hide()
				end
				header:Hide()
				if SmallFrame.collapseHeader then 
					SmallFrame.collapseHeader:Hide()
				end
			end	
		end
