...
function Ants:Initialize()
	...
	local function ShouldMapTypeShowAnts()
		local mapType = C_Map.GetMapInfo(WorldMapFrame:GetMapID()).mapType
			...
			for index, waypoint in ipairs(DGV.DugisArrow.waypoints) do
				...
				if not (new_x == last_x and new_y == last_y) then
					...
					if x1 --[[and x1~=x2 and y1~=y2]] then
						...
						if DugisGuideViewer:UserSetting(DGV_SHOWANTS) 
							and not DugisGuideViewer.WrongInstanceFloor 
							and len > 0.0001 
							and ShouldMapTypeShowAnts() 
						then
