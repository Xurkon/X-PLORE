...
function AutoSell:Initialize()
	function MOD:MERCHANT_SHOW (event)
		if DugisGuideViewer:UserSetting(DGV_AUTOSELL) then
				...
				for slot = 1, C_Container.GetContainerNumSlots(bagId) do
					...
					if link then
						if string.find(link,"|cff9d9d9d") then
							local merchantPrice =  select(11, C_Item.GetItemInfo(link))
			...
			if total > 0 then
				DEFAULT_CHAT_FRAME:AddMessage(string.format("|cff11ff11Dugi Guides: |rGrey quality items sold for %s", C_CurrencyInfo.GetCoinTextureString(total)))
			end
		end
	end
