...
function XP:PopulateFeaturedView(parent, dataset)
    ...
    if dataset == "leveling" then
        -- Show test guides or first few leveling guides
        local count = 0
    ...
    for i, guide in ipairs(featuredGuides) do
        ...
        self:ApplyBackdrop(card, "panel", "bg_medium", "border")
        ...
        self:ApplyFont(title, "bold", "text_bright")
        ...
        self:ApplyFont(info, "small", "text_muted")
        ...
        -- Click to load guide
        card:SetScript("OnClick", function()
            self.Tabs:LoadGuideToTab(guide.id)
            self:Print("Loaded guide: " .. (guide.title or guide.id))
            if self.MenuFrame then self.MenuFrame:Hide() end
            if self.ViewerFrame then self.ViewerFrame:Show() end
        end)
    ...
    if #featuredGuides == 0 then
        ...
        self:ApplyFont(placeholder, "small", "text_dim")
...
function XP:CreateInlineOptions(parent)
    ...
    btnRow:SetHeight(48)
    btnRow:SetPoint("BOTTOMLEFT", contentArea, "BOTTOMLEFT", 0, 0)
    btnRow:SetPoint("BOTTOMRIGHT", contentArea, "BOTTOMRIGHT", 0, 0)
